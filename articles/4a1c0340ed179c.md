...
&emsp;`fragColor = vec4(1 - tex.rgb, tex.a);`で、1から画像のRGBの値を引いた値を代入しています。これにより画像の濃淡が逆になります。\
...
# クロスフフェード
&emsp;2枚の画像を使ってクロスフェードを作成しましょう。`flutter_log.png`と`cross_fading.frag`を用意します。
...
void main() {
    vec2 uv = FlutterFragCoord().xy / uSize;
    vec4 tex1 = texture(uTexture1, uv.xy).rgba;
    vec4 tex2 = texture(uTexture2, uv.xy).rgba;
